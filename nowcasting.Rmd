...
```
<br>En un segundo modelo nos interesa tomar en cuenta la estacionalidad con periodo fijo de 12 y es  muy claro como hay mejoría de predicción, el intervalo de confianza se hace más pequeño.
```{r}
...
Un tercer modelo contempla un componente de autocorelación 2 basado en las gráficas de autocorrelación y autocorrelación parcial previamente modstradas.

```{r}
ss3 <- AddAr(ss2, data_housing$HSN1FNSA, lags = 2)
...
pred2 <- predict(model3, horizon = 3)
plot(pred2, plot.original = 102)
```
...
CompareBstsModels(list("Modelo - trend" = model1,
                       ...
                       "Modelo - trend + season + AR2" = model3,
                       "Modelo - trend + season + AR2 + vars" = model4),
